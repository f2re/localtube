...
echo '[5/9] build all bootstrap packages'
python3 scripts/build_release.py >/tmp/localtube-build.txt

echo '[6/9] verify macOS/Linux/Windows packages'
python3 scripts/verify_release.py

echo '[7/9] security regression checks'
python3 scripts/static_audit.py

echo '[8/9] fallback installer isolation'
_tmp_rc="$(mktemp -d)"
...
echo '[9/9] version/platform consistency'
V="$(cat app/VERSION)"
